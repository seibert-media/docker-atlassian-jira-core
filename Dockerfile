##############################################################################
# Dockerfile to build Atlassian JIRA Core container images
...
ADD https://www.atlassian.com/software/jira/downloads/binary/atlassian-jira-core-$VERSION.tar.gz /tmp

RUN set -x \
  && tar xvfz /tmp/atlassian-jira-core-$VERSION.tar.gz --strip-components=1 -C /opt/atlassian/jira \
